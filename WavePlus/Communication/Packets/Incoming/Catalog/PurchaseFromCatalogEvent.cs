    ...
    public class PurchaseFromCatalogEvent : IPacketEvent
    {
        public void Parse(GameClient session, ClientPacket packet)
        {
            ...
            if (item.IsLimited) {
                ...
                int limitSells = item.LimitedEditionSells;
                int catalogItemId = item.Id;
                using (WavePlusContext db = PlusEnvironment.GetDbContext()) {
                    db.CatalogItems.Where(c => c.Id == catalogItemId).ExecuteUpdate(s => s.SetProperty(c => c.LimitedSells, limitSells));
                }
            ...
            if (item.Data?.ItemName != null
                && item.Data.ItemName.StartsWith("vip_", StringComparison.OrdinalIgnoreCase)
                && int.TryParse(item.Data.ItemName.AsSpan(4), out int vipSeconds) && vipSeconds > 0) {
                session.GetHabbo().GrantVip(vipSeconds * amountPurchase);
