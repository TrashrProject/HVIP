...
import static com.eu.habbo.habbohotel.rooms.pathfinding.impl.PathUtils.debugPath;
...
public class AdjacentTileFinder {
  ...
  public static final float MAX_DISTANCE_THRESHOLD = 1.5f;
  ...
  public static RoomTile findBestCandidate(PathfinderContext context, RoomTile blockedTile,
      Set<RoomTile> adjacentToBlocked, Set<RoomTile> adjacentToCurrent, RoomUnit roomUnit) {

    Set<Long> currentTileKeys = new HashSet<>();
    for (RoomTile tile : adjacentToCurrent) {
      if (tile != null) {
        currentTileKeys.add(generateTileKey(tile.x, tile.y));
      }
    }

    double bestDistance = Double.MAX_VALUE;
    double bestFCost = Double.MAX_VALUE;
    RoomTile bestCandidate = null;
    RoomTile currentTile = context.getNewTile();
    RoomTile nextTile = context.getGoalLocation();

    for (RoomTile candidateFromBlocked : adjacentToBlocked) {
      if (isInvalidCandidate(context, blockedTile, candidateFromBlocked, currentTile)) {
        continue;
      }

      long tileKey = generateTileKey(candidateFromBlocked.x, candidateFromBlocked.y);
      if (currentTileKeys.contains(tileKey)) {
        double distanceToNext = candidateFromBlocked.distance(nextTile);
        if (distanceToNext <= 2.5f && isCheaper(distanceToNext, bestDistance)) {
          debugPath(roomUnit.getRoom().getHabboByRoomUnitId(roomUnit.getId()).getClient(),
              Collections.singleton(candidateFromBlocked));
          bestDistance = distanceToNext;
          bestCandidate = candidateFromBlocked;
        }
      }
    }
  ...
  private static boolean isCheaper( double distanceToNext,
      double bestDistance) {
    return distanceToNext < bestDistance;
  }

  private static boolean isInvalidCandidate(PathfinderContext context, RoomTile blockedTile,
      RoomTile candidateFromBlocked, RoomTile currentTile) {
    if (candidateFromBlocked == null || candidateFromBlocked.equals(currentTile)
        || candidateFromBlocked.equals(blockedTile) || !candidateFromBlocked.isWalkable()) {
      return true;
    }

    boolean isDiagonalMove = isDiagonalMove(currentTile, candidateFromBlocked);
    return isDiagonalMove && isBlockedDiagonal(context, currentTile.getX(), currentTile.getY(),
        candidateFromBlocked.getX(), candidateFromBlocked.getY());
  }

  private static boolean isDiagonalMove(RoomTile from, RoomTile to) {
