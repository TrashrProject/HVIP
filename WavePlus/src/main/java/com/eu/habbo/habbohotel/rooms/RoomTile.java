...
public class RoomTile {
    ...
    public short relativeHeight() {
        ...
        } else if (!this.allowStack && (this.state == RoomTileState.BLOCKED
            || this.state == RoomTileState.SIT)) {
            return 64 * 256;
        }
    ...
    public void setgCosts(RoomTile previousRoomTile) {
        this.setgCosts(previousRoomTile,
            this.diagonally ? RoomLayout.DIAGONALMOVEMENTCOST : RoomLayout.BASICMOVEMENTCOST);
    }
    ...
    public void sethCosts(RoomTile parent, int cost) {
        this.hCost = (short) ((Math.abs(this.x - parent.x) + Math.abs(this.y - parent.y)) * (cost));
    }

    public String toString() {
        return "RoomTile (" + this.x + ", " + this.y + ", " + this.z + "): h: " + this.hCost
            + " g: " + this.gCost + " f: " + this.getfCosts();
    }
