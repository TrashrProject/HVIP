...
public class RoomLayout {
  ...
  public RoomLayout(ResultSet set, Room room) throws SQLException {
    ...
    try {
      ...
      this.pathfinder = new PathfinderImpl(this.room, MAXIMUM_STEP_HEIGHT,
          ...
          Emulator.getConfig().getBoolean("pathfinder.retro-style.diagonals", true));
    } catch (Exception e) {
  ...
  public void parse() {
      ...
      for (short x = 0; x < this.mapSizeX; x++) {
        ...
        if (square.equalsIgnoreCase("x")) {
          ...
          if (square.isEmpty()) {
            ...
            height = (short) (10 + "ABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(
                square.toUpperCase()));
          }
        }
        this.mapSize += 1;
    ...
    if (this.doorTile != null) {
      ...
      if (doorFrontTile != null && this.tileExists(doorFrontTile.x, doorFrontTile.y)) {
        if (this.roomTiles[doorFrontTile.x][doorFrontTile.y].state
  ...
  public boolean fitsOnMap(RoomTile tile, int width, int length, int rotation) {
    ...
    if (rotation == 0 || rotation == 4) {
      return !fitsOnSquare(tile, length, width);
    } else if (rotation == 2 || rotation == 6) {
      return !fitsOnSquare(tile, width, length);
    }

    return true;
  }

  private boolean fitsOnSquare(RoomTile tile, int width, int length) {
  ...
  public THashSet<RoomTile> getTilesAt(RoomTile tile, int width, int length, int rotation) {
    ...
    if (rotation == 0 || rotation == 4) {
      getTilesAtSquare(tile, width, length, pointList);
    } else if (rotation == 2 || rotation == 6) {
      getTilesAtSquare(tile, length, width, pointList);
    }

    return pointList;
  }

  private void getTilesAtSquare(RoomTile tile, int width, int length,
      THashSet<RoomTile> pointList) {
