...
public class InteractionTeleport extends HabboItem {
  ...
  public InteractionTeleport(int id, int userId, Item item, String extradata, int limitedStack,
      int limitedSells) {
  ...
  private void tryTeleport(GameClient client, Room room) {
    ...
    if (this.roomUnitID == unit.getId() && unit.getCurrentLocation().equals(currentLocation)) {
    ...
    } else if (unit.getCurrentLocation().equals(currentLocation) || unit.getCurrentLocation()
        .equals(infrontTile)) {
      ...
      unit.setGoalLocation(infrontTile);
      Emulator.getThreading()
          .run(new RoomUnitWalkToLocation(unit, infrontTile, room, onSuccess, onFail));
    }
  }
