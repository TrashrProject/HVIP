...
public class InteractionOneWayGate extends HabboItem {
  ...
  public InteractionOneWayGate(int id, int userId, Item item, String extradata, int limitedStack,
      int limitedSells) {
  ...
  public void onClick(final GameClient client, final Room room, Object[] objects) throws Exception {
    ...
    if (client != null) {
      RoomTile tileInfront = room.getLayout()
          .getTileInFront(room.getLayout().getTile(this.getX(), this.getY()), this.getRotation());
        if (tileInfront == null) {
            return;
        }
        ...
        if (!currentLocation.hasUnits()) {
          ...
          onSuccess.add(() -> {
            ...
            RoomTile tile = room.getLayout()
                .getTileInFront(room.getLayout().getTile(this.getX(), this.getY()),
                    this.getRotation() + 4);
            unit.setGoalLocation(tile);
            Emulator.getThreading()
                .run(new RoomUnitWalkToLocation(unit, tile, room, onFail, onFail));

            Emulator.getThreading().run(
                () -> WiredHandler.handle(WiredTriggerType.WALKS_ON_FURNI, unit, room,
                    new Object[]{this}), 500);
          });
          ...
          Emulator.getThreading()
              .run(new RoomUnitWalkToLocation(unit, currentLocation, room, onSuccess, onFail));
