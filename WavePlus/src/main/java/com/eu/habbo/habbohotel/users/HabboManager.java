...
public class HabboManager {
  ...
  public void addHabbo(Habbo habbo) {
    ...
    this.usernameToId.put(habbo.getHabboInfo().getUsername().toLowerCase(),
        habbo.getHabboInfo().getId());
  }

  public void removeHabbo(Habbo habbo) {
    this.onlineHabbos.remove(habbo.getHabboInfo().getId());
    this.usernameToId.remove(habbo.getHabboInfo().getUsername().toLowerCase());
  }
  ...
  public Habbo getHabbo(String username) {
    Integer id = this.usernameToId.get(username.toLowerCase());
    if (id != null) {
      Habbo habbo = this.onlineHabbos.get(id);
      // Double-check username in case it has changed
      if (habbo != null && habbo.getHabboInfo().getUsername().equalsIgnoreCase(username)) {
        return habbo;
      } else if (habbo != null) {
        for (Habbo other : this.onlineHabbos.values()) {
          if (other.getHabboInfo().getUsername().equalsIgnoreCase(username)) {
            return other;
          }
        }
      }
        ...
        if (set.next()) {
          ...
          if (!Emulator.debugging) {
            try (PreparedStatement stmt = connection.prepareStatement(
                "UPDATE users SET auth_ticket = ? WHERE id = ? LIMIT 1")) {
              stmt.setString(1, "");
...
//
  ...
  public List<Map.Entry<Integer, String>> getNameChanges(int userId, int limit) {
    ...
    try (Connection connection = Emulator.getDatabase().getDataSource()
      ...
      try (ResultSet set = statement.executeQuery()) {
        while (set.next()) {
          nameChanges.add(
              new AbstractMap.SimpleEntry<>(set.getInt("timestamp"), set.getString("new_name")));
        }
      }
    } catch (SQLException e) {
      LOGGER.error("Caught SQL exception", e);
  ...
  public void setRank(int userId, int rankId) throws Exception {
    ...
    if (habbo != null && habbo.getHabboStats() != null) {
      ...
      habbo.alert(Emulator.getTexts().getValue("commands.generic.cmd_give_rank.new_rank")
  ...
  public void staffAlert(String message) {
    message =
        Emulator.getTexts().getValue("commands.generic.cmd_staffalert.title") + "\r\n" + message;
    ServerMessage msg = new GenericAlertComposer(message).compose();
    Emulator.getGameEnvironment().getHabboManager()
        .sendPacketToHabbosWithPermission(msg, "cmd_staffalert");
  }
}