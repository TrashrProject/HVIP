...
public class HandleDoorbellEvent extends MessageHandler {
  ...
  public void handle() throws Exception {
    if (this.client.getHabbo().getHabboInfo().getCurrentRoom() != null && this.client.getHabbo()
        .getHabboInfo().getCurrentRoom().hasRights(this.client.getHabbo())) {
      String username = this.packet.readString();
      boolean accepted = this.packet.readBoolean();

      Habbo habbo = Emulator.getGameServer().getGameClientManager().getHabbo(username);

      if (habbo != null && habbo.getHabboInfo().getRoomQueueId() == this.client.getHabbo()
          .getHabboInfo().getCurrentRoom().getId()) {
        ...
        if (accepted) {
          ...
          Emulator.getGameEnvironment().getRoomManager()
              .enterRoom(habbo, this.client.getHabbo().getHabboInfo().getCurrentRoom().getId(), "",
                  true);
        } else {
