...
class TeleportActionFive implements Runnable {
  ...
  public void run() {
    ...
    RoomTile currentLocation = this.room.getLayout()
        .getTile(this.currentTeleport.getX(), this.currentTeleport.getY());
    RoomTile tile = this.room.getLayout()
        .getTileInFront(currentLocation, this.currentTeleport.getRotation());

    if (tile != null) {
      List<Runnable> onSuccess = new ArrayList<>();
      onSuccess.add(() -> Emulator.getThreading().run(() -> unit.isLeavingTeleporter = false, 300));

      unit.setGoalLocation(tile);
      unit.statusUpdate(true);
      unit.isLeavingTeleporter = true;
      Emulator.getThreading()
          .run(new RoomUnitWalkToLocation(unit, tile, room, onSuccess, onSuccess));
    }
    ...
    if (teleportTile != null && teleportTile instanceof InteractionTeleportTile
        && teleportTile != this.currentTeleport) {
      try {
