<div class="content"><div class="container">
    <div class="profession-page-heading">
        <div><small>PARADISE ROLEPLAY</small><h1>Métiers</h1><p>Retrouve les entreprises et services publics de la ville.</p></div>
        <span class="profession-heading-icon"><i class="fas fa-briefcase"></i></span>
    </div>
    <div class="corporation-index-grid profession-index-grid">
    <?php
    // La page Corporations est alimentée directement par le vrai système de métiers RP.
    // Tout nouveau métier actif ajouté dans `jobs` (dont Zy'Croque et Tasty Crousty)
    // apparaît automatiquement ici sans liste PHP à maintenir.
    $Jobs = $DB->Query("SELECT j.id,j.name,j.display_name,j.description,
        ...
        WHERE j.active=1 AND j.name <> 'unemployed'
    ...
    ?>
        <div class="corp profession-card"><div class="content-box">
            <div class="title"><?php echo htmlspecialchars($name,ENT_QUOTES,'UTF-8'); ?><span class="title-small float-right"><small><?php echo (int)$Job['employee_count']; ?> employé(s)</small></span></div>
            <div class="box-content profession-card-body">
                ...
                <div class="profession-card-copy">
                    ...
                    <span><?php echo htmlspecialchars($Job['description'] ?: 'Métier disponible en ville',ENT_QUOTES,'UTF-8'); ?></span>
                    <small><?php echo (int)$Job['rank_count']; ?> grade(s) disponible(s)</small>
                </div>
            </div>
        </div></div>
    <?php endwhile; else: ?>
        <div class="content-box"><div class="title">Aucun métier</div><div class="box-content">Aucun métier n'est disponible actuellement.</div></div>
    <?php endif; ?>
    </div>
</div></div>
