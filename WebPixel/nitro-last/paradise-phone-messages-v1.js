(() => {
  ...
  const VERSION = '1.0.1-messages-v1';
  ...
  function enhanceConversationRows(page) {
    ...
    page.querySelectorAll('.pp-list [data-pp-conversation]').forEach(node => {
      ...
      if (meta) {
        const next = formatListDate(row.last_at);
      ...
      if (name) {
        const next = clean(row.name) || clean(row.number);
  ...
  function simplifyEmptyState(page) {
    ...
    const subtitle = empty.querySelector('span');
    if (strong && strong.textContent !== 'Aucune conversation') strong.textContent = 'Aucune conversation';
    if (subtitle && subtitle.textContent !== 'Commencez une discussion avec un contact.') subtitle.textContent = 'Commencez une discussion avec un contact.';
  }
  ...
  function renderNewScreen() {
    ...
    const newBody = screen.querySelector('[data-pp-msg-new-body]');
    if (!newBody) return;
    const contact = selectedContactNumber ? findContactByNumber(selectedContactNumber) : null;
    const renderedFor = newBody.dataset.renderedFor || '';
    const targetKey = contact ? clean(contact.number) : '__picker__';
    if (renderedFor === targetKey) return;
    newBody.dataset.renderedFor = targetKey;
    if (contact) renderNewComposer(newBody, contact);
    else renderContactPicker(newBody);
  }
  ...
  function enhanceList(page) {
    ...
    page.querySelectorAll('[data-pp-new-message-sheet]').forEach(sheet => {
      sheet.hidden = true;
  ...
  async function sendFirstMessage(form) {
    ...
    try {
      ...
      if (conversation?.phone_id) openExistingConversation(conversation.phone_id);
        ...
        const newBody = form.closest('[data-pp-msg-new-body]');
        if (newBody) newBody.dataset.renderedFor = '';
        schedule();
      }
    } catch (error) {
      const newBody = form.closest('[data-pp-msg-new-body]');
      if (newBody) {
        let errorNode = newBody.querySelector('.pp-msg-inline-error');
        if (!errorNode) {
          errorNode = document.createElement('div');
          errorNode.className = 'pp-msg-inline-error';
          newBody.insertBefore(errorNode, form);
        }
  ...
  function onClick(event) {
    ...
    if (contactRow && phoneRoot()?.contains(contactRow)) {
      ...
      if (existing?.phone_id) openExistingConversation(existing.phone_id);
        ...
        const newBody = phoneContent()?.querySelector('[data-pp-msg-new-body]');
        if (newBody) newBody.dataset.renderedFor = '';
        renderNewScreen();
      }
    }
