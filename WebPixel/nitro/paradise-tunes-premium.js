(() => {
    ...
    class ParadiseTunes {
        constructor() {
            ...
            this.tab = 'home'; this.filter = 'tracks'; this.query = ''; this.overlay = null; this.menuTrack = null;
        ...
        async play(track, queue=this.tracks, force=false) { if(!track)return; this.queue=queue;this.queueIndex=Math.max(0,queue.findIndex(t=>t.id===track.id)); if(!force&&this.current?.id===track.id){return this.toggle();} this.current=track;this.audio.src=track.audioUrl;this.addRecent(track.id);try{await this.audio.play();}catch{this.notice('Cette source audio n’est pas compatible.');}this.render(); }
        toggle(){if(!this.current)return;this.audio.paused?this.audio.play().catch(()=>this.notice('Lecture impossible.')):this.audio.pause();}
        ...
        async click(event) {
            ...
            if(el.dataset.ptRetry!==undefined)return this.mountRetry(); if(el.dataset.ptTab){this.tab=el.dataset.ptTab;this.overlay=null;return this.render();}
    ...
    const scan = root => { const nodes=[...(root.matches?.('.ppr-tunes')?[root]:[]),...root.querySelectorAll?.('.ppr-tunes')||[]];nodes.forEach(node=>app.mount(node)); };
