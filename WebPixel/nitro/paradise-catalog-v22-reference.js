(() => {
  ...
  {
    const candidates = [ ...document.querySelectorAll('.nitro-purse-container .nitro-purse-button') ]
      .map(node => text(node))
      .map(value => value.match(/[\d][\d\s.,]*/)?.[0]?.trim() || '')
      .filter(Boolean);

    return {
      credits: candidates[0] || '—',
      diamonds: candidates[1] || '—'
    };
  }

  function findTopNav(root)
  {
    return root.querySelector(':scope > .nitro-card-tabs, :scope > [class*="card-tabs"], .nitro-card-tabs');
  }
  ...
  function ensureBanner(root)
  {
    ...
    if(!banner)
    {
      ...
      banner.innerHTML = `
        ...
        <div class="prc22-tagline">Des milliers de furnis<br>pour rendre votre ville unique !</div>
        <div class="prc22-wallets">
          <div class="prc22-wallet prc22-wallet-credits"><span class="prc22-wallet-icon">🪙</span><span class="prc22-wallet-value">—</span><span class="prc22-wallet-add">+</span></div>
          <div class="prc22-wallet prc22-wallet-diamonds"><span class="prc22-wallet-icon">💎</span><span class="prc22-wallet-value">—</span><span class="prc22-wallet-add">+</span></div>
        </div>
  ...
  function decorateStructure(root)
  {
    ...
    add(navigation, 'prc22-category-panel');
    add(navContainer, 'prc22-category-wrap');
    add(sideColumn, 'prc22-side-column');
    ...
    if(mainGrid)
    {
      contentColumn = [ ...mainGrid.children ].find(node => node !== sideColumn && node instanceof HTMLElement) || null;
    }
    ...
    add(itemGrid, 'prc22-grid');
    itemCells.forEach(cell => add(cell, 'prc22-item'));

    let innerGrid = null;
    if(contentColumn)
    {
      innerGrid = [ ...contentColumn.querySelectorAll('.grid') ].find(grid => grid.contains(firstCell)) || null;
    }
    ...
    add(gridColumn, 'prc22-grid-column');
    add(previewColumn, 'prc22-preview-column');
    ...
    const purchase = root.querySelector('.nitro-catalog-purchase-component,[class*="catalog-purchase"],[class*="purchase-component"]') ||
      buyButtons[0]?.closest('[class*="purchase"],.d-flex.flex-column') || null;
  ...
  function ensureStaffPromo(root, sideColumn)
  {
    ...
    promo.innerHTML = `
      <span class="prc22-staff-crown">👑</span>
      <span class="prc22-staff-copy"><strong>Catalogue Staff</strong><small>Accès aux furnis exclusifs</small></span>
      <span class="prc22-staff-arrow">›</span>`;
  ...
  function boot()
  {
    ...
    new MutationObserver(mutations =>
    {
      ...
      if(refreshExisting)
      {
        document.querySelectorAll(ROOT).forEach(root => window.setTimeout(() => decorate(root), 30));
      }
