-- ParadiseRP V6 - passe finale des familles de blocs (schema legacy)
-- Cette migration doit etre executee APRES toutes les taxonomies V4/V5.
-- Elle ne traite que les mobis clairement identifies comme blocs/formes de construction.
SET NAMES utf8mb4;
START TRANSACTION;
...
SET ci.page_id=CASE
    WHEN CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ci.item_ids, ',', 1), ':', 1) AS UNSIGNED)=5480 THEN 9968141
    WHEN CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ci.item_ids, ',', 1), ':', 1) AS UNSIGNED) IN (5466,996661582) THEN 9968150
    WHEN CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ci.item_ids, ',', 1), ':', 1) AS UNSIGNED)=996700070 THEN 9968140
...
ci.club_only='0'
WHERE
    CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ci.item_ids, ',', 1), ':', 1) AS UNSIGNED) IN (5480,5466,996661582,996700070)
    OR CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ci.item_ids, ',', 1), ':', 1) AS UNSIGNED) BETWEEN 996661787 AND 996661818
...
-- Verrouillage explicite des quatre blocs critiques.
UPDATE catalog_items SET page_id=9968141,have_offer='1',club_only='0'
WHERE CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(item_ids, ',', 1), ':', 1) AS UNSIGNED)=5480;
UPDATE catalog_items SET page_id=9968150,have_offer='1',club_only='0'
WHERE CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(item_ids, ',', 1), ':', 1) AS UNSIGNED) IN (5466,996661582);
UPDATE catalog_items SET page_id=9968140,have_offer='1',club_only='0'
