...
const missing=[];
for(const lib of libs){const id=String(lib?.id||'');if(!id)continue;const file=id.split('*')[0]+'.nitro';if(!fs.existsSync(path.join(figureDir,file)))missing.push({id,file,parts:Array.isArray(lib.parts)?lib.parts.length:0});}
const unique=[...new Map(missing.map(x=>[x.file.toLowerCase(),x])).values()].sort((a,b)=>a.file.localeCompare(b.file));
console.log(`FigureMap libraries: ${libs.length}`);console.log(`Assets .nitro manquants: ${unique.length}`);
...
const report=path.join(path.dirname(figureMapPath),'MissingFigureAssets.txt');fs.writeFileSync(report,unique.map(x=>x.file).join('\r\n'),'utf8');console.log(`Rapport: ${report}`);
