...
$toolsDir=Join-Path $Root '_tools'; $sourceDir=Join-Path $toolsDir 'sphynxkitten-nitro-assets'; $timestamp=Get-Date -Format 'yyyyMMdd-HHmmss'; $backupDir=Join-Path $Root "_backups\clothes-sphynxkitten\$timestamp"
foreach($path in @($figureDir,$currentFD,$currentFM,$mergeScript,$repairScript,$scanScript)){ if(-not(Test-Path $path)){ throw "Introuvable: $path" } }
...
if(-not(Test-Path(Join-Path $sourceDir '.git'))){
 Write-Host '[2/7] Telechargement de sphynxkitten/nitro-assets...' -ForegroundColor Cyan
 git clone --depth 1 --filter=blob:none --sparse --branch master https://github.com/sphynxkitten/nitro-assets.git $sourceDir; if($LASTEXITCODE-ne 0){throw "git clone a echoue: $LASTEXITCODE"}
 git -C $sourceDir sparse-checkout set 'clothes/nitro' 'clothes/json'; if($LASTEXITCODE-ne 0){throw "sparse-checkout a echoue: $LASTEXITCODE"}
}else{
 Write-Host '[2/7] Mise a jour de sphynxkitten/nitro-assets...' -ForegroundColor Cyan
 git -C $sourceDir fetch --depth 1 origin master; if($LASTEXITCODE-ne 0){throw "git fetch a echoue: $LASTEXITCODE"}; git -C $sourceDir reset --hard origin/master; if($LASTEXITCODE-ne 0){throw "git reset a echoue: $LASTEXITCODE"}
}
$sourceFigure=Join-Path $sourceDir 'clothes\nitro'; $sourceFD=Join-Path $sourceDir 'clothes\json\FigureData.json'; $sourceFM=Join-Path $sourceDir 'clothes\json\FigureMap.json'
foreach($path in @($sourceFigure,$sourceFD,$sourceFM)){if(-not(Test-Path $path)){throw "Pack incomplet: $path"}}
Write-Host '[3/7] Copie des .nitro manquants...' -ForegroundColor Cyan
...
Write-Host '[6/7] Scan des bibliotheques manquantes...' -ForegroundColor Cyan; node $scanScript $currentFM $figureDir; if($LASTEXITCODE-ne 0){throw "Scan echoue: $LASTEXITCODE"}
