...
{
    // Restored from the canonical PlusEMU implementation used by ChatEvent.
    public sealed class ChatlogEntry
    {
        ...
        {
            if (_playerReference != null && _playerReference.IsAlive)
                return (Habbo)_playerReference.Target;
        ...
        {
            if (_roomReference != null && _roomReference.IsAlive)
            {
                Room RoomObj = _roomReference.Target as Room;
                if (RoomObj == null || RoomObj.mDisposed)
                    return null;
                return RoomObj;
            }
            return null;
