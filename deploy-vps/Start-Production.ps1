...
do {
 ...
 $gameListener=Get-NetTCPConnection -State Listen -LocalPort 2096 -ErrorAction SilentlyContinue
} until($gameListener -or (Get-Date) -ge $deadline)
$listeners=Get-NetTCPConnection -State Listen|Where-Object{$_.LocalPort-in 2096,2097}|Select-Object LocalAddress,LocalPort,OwningProcess
$listeners
if(-not $gameListener){
 Write-Host 'WavePlus ne repond pas apres 60 secondes. Dernieres lignes du journal :' -ForegroundColor Red
 Get-Content (Join-Path $logs 'waveplus.out.log') -Tail 30 -ErrorAction SilentlyContinue
 Get-Content (Join-Path $logs 'waveplus.err.log') -Tail 30 -ErrorAction SilentlyContinue
 exit 1
}